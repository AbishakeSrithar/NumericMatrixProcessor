...
def inverse():
    ...
    if int(my_dimensions_1[0]) == 1:
        return 1 / rows_1[0][0]

    if int(my_dimensions_1[0]) == 2:
        x = 1 / ans
        print(f"x i s{x}")
        return [[rows_1[1][1] * x, -rows_1[0][1] * x], [-rows_1[1][0] * x, rows_1[0][0] * x]]
    
    g = cofactors(rows_1, int(my_dimensions_1[0]))

    rows_3 = [[None]*int(my_dimensions_1[0]) for x in range(int(my_dimensions_1[0]))]
    for i in range(int(my_dimensions_1[0])):
        for j in range(int(my_dimensions_1[0])):
            rows_3[i][j] =float(g[j][i])
            if rows_3[i][j] % 1 == 0:
                rows_3[i][j] = int(rows_3[i][j])
...
def cofactors(matrix, n):
    lst_C = [[None]*n for x in range(n)]
    for i in range(n):
        for j in range(n):
            new_matrix = copy.deepcopy(matrix)
            new_matrix.pop(i)
            for k in range(n - 1):
                new_matrix[k].pop(j)
            lst_C[i][j] = (-1) ** (i + j) * det(new_matrix, n - 1)
    return lst_C
